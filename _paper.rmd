...
(ref:caseames) Exploring an instance with a large residual as the PI from fitting sales price [USD] to other variables in the Ames housing 2018. The sale price of the PI was under-predicted. The local explanation indicates a sizable attribution to lot area (`LtA`). The CI has a similar predicted sales price and smaller residual, and has very little attribution to lot area. In the attribution projection, the PI has a higher sales price than the CI. Reducing the contribution of lot area brings these two prices in line. This suggests if lot area was not considered by the model, then the two houses would be quite similar. That is, the large residual is due to a lack of factoring in the lot area for the prediction of PI's sales price. (A video showing the animation is at [vimeo.com/666431134](https://vimeo.com/666431134).)
...
New tools to assist with the interpretability of black-box models is increasingly important. This paper provides a technique that builds on local interpretations to explore the feature importance local to an instance. The local interpretations form an attribution projection from which feature contributions are varied using a radial tour. Several diagnostic plots are provided to assist with understanding the sensitivity of the prediction to particular features. A global view shows the data space, explanation space, model fit. The user can interactively select instances to compare and contrast, and study further. 

The approach has been illustrated on four data examples using random forest models and using the tree SHAP local explanation. In the penguins example, we showed how the misclassification of a penguin arose due to it having an unusually small flipper size compared to others of it's species. This was verified by making a follow-up plot of the data including this variable. In the chocolates example, we showed how a dark chocolate was misclassified primarily due to it value on Fiber, and a milk chocolate misclassified as dark due to it's lowish Sodium value. For the FIFA example, we show how low Messi's salary would go if it depended on defensive skills. In the Ames housing data, an inaccurate prediction for a house was likely due to lot area not being effectively used. 

The approach is manually intensive, and thus only feasible for investigating a few instances. The approach is to select instances which the model hasn't done well and compare it with an instance where the model did fit well.  The radial tour launches from the attribution projection, to enable exploration of the sensitivity of the prediction to any feature. It can be helpful to make additional plots of the features and response in order to cross-check interpretations made from the cheem viewer. This methodology provides an additional tool in the box for studying model fitting. 

An implemenation is provided in the open-source __R__ package __cheem__, available on [CRAN](https://CRAN.R-project.org/package=cheem). Example data sets are provided, and you can upload your own data after model fitting and computing the local explanaitons. In theory this approach would work with any black box model but the implementation currently only calculates tree SHAP for tree-based models supported by __treeshap__ (tree based models from __randomForest__, __ranger__, __gbm__, __xgboost__, __lightgbm__, or __catboost__). Tree SHAP was selected because of its computational efficiency. The SHAP and oscillation explanations could be added with use of the `DALEX::explain()` and would be an excellent direction to extend the work [@biecek_dalex_2018; @biecek_explanatory_2021]. 
