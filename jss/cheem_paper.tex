...
predictors to arrive at a prediction. ``Local explanations'', which
provide a point estimate of the linear variable importance in the
vicinity of one observation, are one XAI method. These can be considered
to be linear projections and can be further explored to understand the
interactions between variables used to make predictions across the
...
calculate local explanations, i.e.~for every observation how strongly
...
This paper provides a technique that builds on local explanations to
explore the variable importance local to an observation. The local
explanations of an attribution projection from which variable
